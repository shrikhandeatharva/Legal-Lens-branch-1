...
# genai.configure(api_key='AIzaSyCqCxhmSr5Px-fu_-5Mo1D1ChmfJlLjGH0')
...
#     # Uncomment and implement the embedding and upsert logic when ready
#     # for chunk in chunks:
#     #     embeddings = voyage.embed(chunk)
#     #     index.upsert(
#     #         vectors=[{
...
# def create_new_chat(user_email: str, table_name: str, chat_name: str | None = None):
#     chat_id = str(uuid4())
#     if not chat_name:
#         chat_name = f"Chat {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
#     supabase.table(table_name).insert(new_chat).execute()
...
#     if not chat_id:
#         chat_id = create_new_chat(user_email, table_name)["chat_id"]
...
#     table_name = "chats_data"
#     new_chat = create_new_chat(user_email, table_name, chat_name)
#     return new_chat

# @app.get("/api/all_chats")
# async def handle_all_chats(user_email: str):
#     table_name = "chats_data"
#     response = supabase.table(table_name).select("chat_id, chat_name, created_at").eq("user_email", user_email).order("created_at", desc=True).execute()
#     return {"chats": response.data}

# @app.put("/api/update_chat_name")
# async def handle_update_chat_name(request: UpdateChatNameRequest):
#     user_email = request.user_email
#     chat_id = request.chat_id
...
#     try:
#         supabase.table(table_name).update({"chat_name": new_name}).eq("chat_id", chat_id).eq("user_email", user_email).execute()
...
def save_query_response(user_email: str, table_name: str, chat_id: str, is_query: bool, content: str):
    chat = supabase.table(table_name).select("*").eq("chat_id", chat_id).single().execute()
    
    if not chat.data:
        raise HTTPException(status_code=404, detail="Chat not found")
    
    chat_data = chat.data.get("data", [])
    chat_data.append({
    ...
    supabase.table(table_name).update({"data": chat_data}).eq("chat_id", chat_id).execute()
    
    print(f"{'Query' if is_query else 'Response'} saved for chat ID: {chat_id}")

def get_chat_history(user_email: str, table_name: str, chat_id: str):
    response = supabase.table(table_name).select("*").eq("chat_id", chat_id).eq("user_email", user_email).single().execute()
    return response.data
...
def generate_chat_name(query: str):
    words = query.split()[:5]  # Take the first 5 words
